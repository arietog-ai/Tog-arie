...
import { byId, nf, nf4, copyText } from './utils.js';

const UNIT_FACTORS = { '': 1, 'M': 1_000_000, 'G': 1_000_000_000, 'T': 1_000_000_000_000 };
...
function attemptRowsHTML() {
  return [1, 2, 3, 4, 5, 6].map(i => {
    ...
    return `
      <div style="display:grid;grid-template-columns:90px 1fr 110px;gap:8px;align-items:center;margin-bottom:8px;">
        <span class="pill" style="text-align:center;font-size:12px;">${day}일차 ${turn}회</span>
        <input type="text" id="eel-dmg-${i}" placeholder="데미지 입력" inputmode="numeric" />
...
function attemptChipsHTML() {
  return [1, 2, 3, 4, 5, 6].map(n => {
    const day = Math.ceil(n / 2), turn = n % 2 === 1 ? 1 : 2;
    return `
...
<section class="container" style="padding-bottom:60px;">
  ...
  <div class="card" style="margin-bottom:12px;font-size:13px;line-height:1.8;">
    ...
    단위: <b>M</b>(×백만) &nbsp;·&nbsp; <b>G</b>(×십억) &nbsp;·&nbsp; <b>T</b>(×조) &nbsp;·&nbsp; 단위없음
  </div>
  ...
  <div id="eel-panel-d2c">
    <div class="card">
      ...
      <div style="text-align:right;margin-top:6px;">
        <button class="hero-btn" id="eel-calc-d2c" style="padding:10px 22px;">⚡ 계산하기</button>
      </div>
    </div>
    <div class="card" id="eel-res-d2c" style="margin-top:12px;display:none;">
      ...
      <button class="hero-btn" id="eel-copy-d2c" style="margin-top:10px;padding:10px 18px;">📋 결과 복사</button>
    </div>
  </div>
  ...
  <div id="eel-panel-c2d" style="display:none;">
    <div class="card">
      ...
      <div style="text-align:right;">
        <button class="hero-btn" id="eel-calc-c2d" style="padding:10px 22px;">⚡ 계산하기</button>
      </div>
    </div>
    <div class="card" id="eel-res-c2d" style="margin-top:12px;display:none;">
      ...
      <button class="hero-btn" id="eel-copy-c2d" style="margin-top:10px;padding:10px 18px;">📋 결과 복사</button>
    </div>
  </div>
</section>`;
  ...
  byId('eel-attempt-sel').addEventListener('click', e => {
    const btn = e.target.closest('.eel-att-btn'); if (!btn) return;
    ...
    try {
      ...
      const tbody = meta.map((r, idx) => `
        <tr style="border-bottom:1px solid var(--line);${r.dmg === 0 ? 'opacity:.32' : ''}">
          ...
          <td style="padding:8px 6px;text-align:right;font-weight:700;">${r.dmg > 0 ? formatWithUnit(r.dmg, r.unit) : '—'}</td>
          <td style="padding:8px 6px;text-align:right;color:var(--ok);font-weight:800;">${contributions[idx] > 0 ? nf(contributions[idx]) : '—'}</td>
        </tr>`).join('');
      byId('eel-res-d2c-body').innerHTML = `
        <table style="width:100%;border-collapse:collapse;font-size:14px;margin-bottom:12px;">
          ...
          <tbody>${tbody}</tbody>
          <tfoot><tr style="background:#1b2230;font-weight:800;">
  ...
  byId('eel-calc-c2d').addEventListener('click', async () => {
    ...
    if (!Number.isFinite(conVal) || conVal <= 0) { alert('올바른 기여도를 입력하세요.'); return; }
    const resUnit = byId('eel-res-unit').value;
    setLoading('eel-calc-c2d', true);
    try {
      const { totalDmg, perAttempt } = await callEelApi({ mode: 'c2d', contribution: conVal, attempts: selectedAttempts });
      const tbody = Array.from({ length: selectedAttempts }, (_, idx) => {
        const i = idx + 1, day = Math.ceil(i / 2), turn = i % 2 === 1 ? 1 : 2;
        return `
          <tr style="border-bottom:1px solid var(--line);">
            <td style="padding:8px 6px;text-align:center;">${day}일차 ${turn}회</td>
            <td style="padding:8px 6px;text-align:right;color:var(--ok);font-weight:700;">${formatWithUnit(perAttempt, resUnit, true)}</td>
          </tr>`;
      }).join('');
      byId('eel-res-c2d-body').innerHTML = `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px;">
          ...
          <div style="padding:12px;background:#0f1420;border-radius:12px;border:2px solid var(--ok);">
            ...
            <div style="font-size:20px;font-weight:800;color:var(--ok);margin-top:4px;">${formatWithUnit(totalDmg, resUnit, true)}</div>
          </div>
        </div>
      ...
      byId('eel-copy-c2d').onclick = () => copyText(copyStr);
    } catch (err) {
      alert(`계산 실패: ${err.message}`);
    } finally {
